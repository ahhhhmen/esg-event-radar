...
class EventItem(BaseModel):
    ...
    exec_value_score: int = Field(
        default=3, ge=1, le=5, description="高管参会价值 1–5，评分准则见 scoring_criteria.md"
    )
    ...
    # ── 字段级宽容校验（Pydantic 校验器：自动归一化 LLM 幻觉值，防止整批中断）────
    # mode="before" 在 Literal 校验前执行，把 virtual/线下 等变体归一为标准枚举值。
    @field_validator("format", mode="before")
    @classmethod
    def _coerce_format(cls, v: str) -> str:
        if v is None:
            return "unknown"
        if v in {"in-person", "online", "hybrid", "unknown"}:
            return v
        # 模糊匹配常见变体
    ...
    def _coerce_fee_tier(cls, v: str) -> str:
        if v is None:
            return "unknown"
        if v in {"free", "paid", "invite-only", "unknown"}:
            return v
        v_lower = str(v).lower().strip()
        mapping = {
    ...
    @field_validator("exec_value_score", mode="before")
    @classmethod
